...
Runtime observation is normalized behind an execution adapter boundary:
...
`CodexExecutionAdapter` is the first implementation. It normalizes platform-specific process discovery into logical execution sessions so helper/app-server processes can remain process-level diagnostics without becoming duplicate execution rows.

The normalized session keeps adapter, logical ID, agent, cwd, repository/worktree roots, and process IDs. Claude Code and OpenCode implementations are intentionally separate follow-up adapters.

See `specs/H14-execution-adapter-contract.md`.
